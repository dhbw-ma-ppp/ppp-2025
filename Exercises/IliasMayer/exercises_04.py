...
def intcode_computer(memory):
    """
    Simuliert den Intcode-Computer mit allen Opcodes und Parameter-Modi.
    """
    # Kopie des Speichers erstellen, um das Original nicht zu verändern
    mem = memory.copy()
    ip = 0  # Instruction Pointer
    
    def get_parameter(offset, mode):
    ...
    while True:
        ...
        if opcode == 99:
            ...
            param1 = get_parameter(1, mode1)
            param2 = get_parameter(2, mode2)
            target = mem[ip + 3]
            mem[target] = param1 + param2
            ip += 4
            ...
            param2 = get_parameter(2, mode2)
            target = mem[ip + 3]
            mem[target] = 1 if param1 < param2 else 0
            ip += 4
            ...
            raise ValueError(f"Unbekannter Opcode: {opcode} bei Position {ip}")

# test1 = [3,9,8,9,10,9,4,9,99,-1,8]  # test whether the input is equal to 8
# test2 = [3,3,1107,-1,8,3,4,3,99]  # test whether the input is less than 8
# test3 = [3,3,1105,-1,9,1101,0,0,12,4,12,99,1]  # test whether the input is 0

print("Intcode-Computer gestartet.")
intcode_computer(commands)
