...
def simulate_computer(liste):
    pos = 0 #start at position 0
    while True:
        opcode = liste[pos] % 100               #last 2 digits = opcode
        mode1 = (liste[pos] // 100) % 10        #3rd last digit = mode of parameter 1
        mode2 = (liste[pos] // 1000) % 10       #4th last digit = mode of parameter 2

        match opcode:
            case 1: #addition
                parameter1 = get_parameter(liste, pos+1, mode1)
                parameter2 = get_parameter(liste, pos+2, mode2)
                target = liste[pos+3]           #target is always in position mode
                liste[target] = parameter1 + parameter2
                pos+=4
            ...
            case 3: #input
                target = liste[pos+1]
                liste[target] = int(input("Please enter a number:"))
                pos += 2
...
def get_parameter(liste, pos, mode):    #returns number depending on mode
    if mode == 0:
        return liste[liste[pos]]        #position mode
    elif mode == 1:
        return liste[pos]               #immediate mode

test1 = [3,9,8,9,10,9,4,9,99,-1,8]
test2 = [3,3,1107,-1,8,3,4,3,99]
test3 = [3,3,1105,-1,9,1101,0,0,12,4,12,99,1]

print("Output:", simulate_computer(commands))
#Output: 16694270