...
def maschine_funktion(speicher):
    ...
    while position < len(speicher):
        ...
        def get_wert(offset, mode):
            parameter = speicher[position + offset]
            if mode == 1:  # Immediate mode
                return parameter
            else:  # Position mode
                return speicher[parameter]
        ...
        if opcode == 99: # Halt
            ...
            val2 = get_wert(2, mode2)
            ziel_pos = speicher[position + 3] 
            speicher[ziel_pos] = val1 + val2
            position += 4
            ...
            ziel_pos = speicher[position + 1]
            eingabe = int(input("Bitte eine Zahl eingeben: "))
            #eingabe = 5  # feste Eingabe für die Aufgabe
            speicher[ziel_pos] = eingabe
            position += 2
            ...
            if val1 != 0:
                position = val2
                # wenn wir springen, dann keine weitere Positionsanpassung
                continue
            position += 3
