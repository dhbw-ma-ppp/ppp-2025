...
def calculation_machine(input_list: list[int], input_value: int) -> int:
    ip = 0  # instruction pointer
    output = None

    def get_param(mode, val):
        if mode == 1:
            return val 
        else:
            return input_list[val]

    while True:
        instruction = input_list[ip]
        opcode = instruction % 100

        # Examines the mode of the first parameter
        ### Example ###
        # 1005 // 100 == 10
        # 10 % 10 = 0 --> mode of the first parameter 
        mode1 = (instruction // 100) % 10
        # Examines thr mode of the second parameter
        ...
        if opcode == 99:
            ...
            first_parameter = get_param(mode1, input_list[ip+1])
            second_parameter = get_param(mode2, input_list[ip+2])
            insertion_point = input_list[ip+3]
            input_list[insertion_point] = first_parameter + second_parameter
            ip += 4
            ...
            insertion_point = input_list[ip+1]
            input_list[insertion_point] = input_value
            ip += 2

        elif opcode == 4:  # output
            first_parameter = get_param(mode1, input_list[ip+1])
            output = first_parameter
            ...
            if first_parameter < second_parameter:
                input_list[insertion_point] = 1 
            else:
                input_list[insertion_point] = 0
            ip += 4
...
# Run the given program with input 5
test_input = 5
result = calculation_machine(commands, test_input)
