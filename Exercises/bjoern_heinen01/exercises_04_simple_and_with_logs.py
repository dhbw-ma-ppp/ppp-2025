...
def calc_virtual_machine(commands:list[int], use_auto_input:bool = False, auto_input:int = None, should_print:bool = True, log_operations:bool = False):
    ...
    def get_args_indexes(args_start_index:int, op_value:str, op_name:str) -> list[int]:
        op_args_count = op_name_to_arg_count[op_name]
    ...
    while True:
        ...
        op_name = op_code_to_op_name[op_value[-2:].replace("0", "")]

        args_index_start = instr_index+1
        op_arg_indexes = get_args_indexes(args_index_start, op_value, op_name)
        ...
        match op_name:
            ...
            case func_names.output:
                ...
                if last_print:
                    raise RuntimeError("The virtual machine should only print once!")
                last_print = v
...
assert calc_virtual_machine([1002,4,3,4,33], should_print=False) == None
...
if input("Do you want the logs? (Y/N) -> ").upper() == "Y":
