...
# caution: since these opcodes expect a variable number of parameters
# the instruction pointer after executing an instruction should no longer always
# increase by four. 
# Instead, it should increase according to the number of parameters that instruction expects.
# If, however, the opcode directly manipulates the instruction pointer (5 and 6)
# there should be no additional modification of the instruction pointer.
...
#
# Additionally, you need to support two different parameter modes: 
# position mode (mode 0), and immediate mode (mode 1).
# Position mode: 
# Opcode arguments are memory addresses. If an argument has the value 18 you fetch the 'calculation value'
...
def rechnen(arr):
    ...
    while(1):
        eingabewert = ["0"]*5 
        z = 0
        for digit in list(str(arr[i])[::-1]):
            try:
                eingabewert[z] = digit
                z += 1
            except IndexError:
                break 
        opcode = "".join(eingabewert[:2])
        if opcode == "99":
            print(f"99 was hit.\nThe progrmam has been halted.\n")
            break
        x = 0
        for nicht_i in eingabewert[2:5]:
            if nicht_i == "0":
                match x:
                    case 0:
                        adresse1 = arr[i + 1]
        ...
        match opcode:
            case "10":
                arr[ziel_adresse] = add(wert1,wert2)

                i+= 4
                
            case "20":
                arr[ziel_adresse] = mul(wert1,wert2)
                i += 4
            case "30":
                ...
                while(1):
                    x = int(input("Enter 5\n"))
                    if x == 5:
                        print("Richtiger input erkannt.\n")
                        break
                    else:
                        print("Du sollst 5 eingeben!!!")
                arr[adresse1] = x
                i +=2
            case "40":
                # - 4: output the value of the single parameter for this opcode.
                #      for example 4,19 would output the value stored at address 19
                    print(f"Opcode 4 occurd. The value is: {wert1}\n")
                    i += 2
            case "50":
                # - 5: jump-if-true: if the first parameter is non-zero,
                ...
                if wert1 != 0:
                    ...
                    i += 3
            case "60":
                # - 6: jump-if-false: if the first parameter is zero,
                #      it sets the instruction pointer to the value from the second parameter.
                #      Otherwise, it does nothing.
                if eq(wert1,0):
                    i = wert2
            ...
            case _:
                print("Nicht annerkannter Opcode")
                break
