...
def simulation_computer(memory): #simulated computer pasted from week 2
    i = 0
    while True:
        full_instruction = str(memory[i]).zfill(5) # Pad with leading zeros for easier parsing
        opcode = int(full_instruction[-2:]) # The last two digits are the opcode
        mode1 = int(full_instruction[-3]) # Mode for the first parameter
        mode2 = int(full_instruction[-4]) # Mode for the second parameter
        mode3 = int(full_instruction[-5]) # Mode for the third parameter (not used for current opcodes, but good practice)
        ...
        def get_param_value(mem, index, mode):
            if mode == 0: # Position mode
                ...
                raise ValueError(f"Unknown parameter mode: {mode} at index {index}")
        ...
        if opcode == 99:
            ...
            target_address = memory[i + 3] # Write operations always use position mode
            ...
            target_address = memory[i + 1] # Write operations always use position mode
            ...
            if val1 != 0:
                ...
                i += 3 # Move past instruction and its two parameters
