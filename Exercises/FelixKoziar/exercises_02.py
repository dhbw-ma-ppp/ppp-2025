...
def simulate_computer(lst):
    ...
    for i in range(0, len(lst)-1, 4):
        try:
            ...
            raise Exception("Incomplete instruction at the end of the list")
        opcode = lst[i]
        match opcode:
            case 1: # addition
                try:
                    ...
                    raise Exception("Invalid memory access during addition")
            case 2: # multiplication
                try:
                    lst[lst[i+3]] = lst[lst[i+1]] * lst[lst[i+2]]
                except IndexError:
                    raise Exception("Invalid memory access during multiplication")
            case 99: # halt
                break
...
commands = [1, 1, 1, 50, 99, 5, 6, 0, 1, 0, 0]
