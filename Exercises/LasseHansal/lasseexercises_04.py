...
def get_value(arr, pos, mode):
    if mode == 0:
        return arr[arr[pos]]
    elif mode == 1:
        return arr[pos]
    else: 
        raise ValueError("Something went wrong with modes")
    
def addition(arr, pos1, pos2, mode1, mode2):
    return get_value(arr, pos1, mode1) + get_value(arr, pos2, mode2)

def multiplication(arr, pos1, pos2, mode1, mode2):
    return get_value(arr, pos1, mode1) * get_value(arr, pos2, mode2)

def jump_if_true(arr, pos1, pos2, mode1, mode2, ip):
    condition = get_value(arr, pos1, mode1)
    target = get_value(arr, pos2, mode2)
...
def jump_if_false(arr, pos1, pos2, mode1, mode2, ip):
    condition = get_value(arr, pos1, mode1)
    target = get_value(arr, pos2, mode2)
...
def check_if_less_than(arr, pos1, pos2, mode1, mode2):
    number1 = get_value(arr, pos1, mode1)
    number2 = get_value(arr, pos2, mode2)
...
def check_if_is_equal(arr, pos1, pos2, mode1, mode2):
    number1 = get_value(arr, pos1, mode1)
    number2 = get_value(arr, pos2, mode2)
...
def oppcode_machine(arr):
    arr = arr.copy()

    ip = 0
    while arr[ip] != 99:
        command = str(arr[ip]).zfill(5)
        oppcode = int(command[-2:])
        ...
        match oppcode:
            case 1:
                result = addition(arr, id1, id2, pos1, pos2)
                write_address = arr[id3]
                arr[write_address] = result
                ip+=4
            case 2: 
                result = multiplication(arr, id1, id2, pos1, pos2)
                write_address = arr[id3]
                arr[write_address] = result
                ip+=4
            case 3:
                result = int(input("Input: "))
                write_address = arr[id1]
                arr[write_address] = result
                ip+=2
            case 4:
                result = get_value(arr, id1, pos1)
                print(result)
                ip+=2
            case 5:
                ip = jump_if_true(arr, id1, id2, pos1, pos2, ip)
            case 6:
                ip = jump_if_false(arr, id1, id2, pos1, pos2, ip)
            case 7:
                result = check_if_less_than(arr, id1, id2, pos1, pos2)
                write_address = arr[id3]
                arr[write_address] = result
                ip+=4
            case 8:
                result = check_if_is_equal(arr, id1, id2, pos1, pos2)
                write_address = arr[id3]
                arr[write_address] = result
                ip+=4
...
test_case1 = [3,9,8,9,10,9,4,9,99,-1,8]
test_case2 = [3,3,1107,-1,8,3,4,3,99]
test_case3 = [3,3,1105,-1,9,1101,0,0,12,4,12,99,1]

oppcode_machine(commands) #Result = 16694270