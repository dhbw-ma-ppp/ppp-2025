...
def get_parameter(memory, pointer, mode_char):
    if mode_char == '1':
        ...
        if 0 <= address < len(memory):
            ...
            logging.error(f"Unvalid memory acces at index {address}")

def translate_instruction(instruction_val):
    instruction_str = str(instruction_val)
    opcode = int(instruction_str[-2:])
    modes_str = instruction_str[:-2].zfill(2) 
    
    return opcode, modes_str

def add(memory, i, modes):
    param1 = get_parameter(memory, i + 1, modes[-1])
    param2 = get_parameter(memory, i + 2, modes[-2])
    
    write_spot = memory[i + 3] 
    memory[write_spot] = param1 + param2
    return i + 4

def multiply(memory, i, modes):
    param1 = get_parameter(memory, i + 1, modes[-1])
    param2 = get_parameter(memory, i + 2, modes[-2])
...
def user_input(memory, i):
    try:
        user_input_str = input("Please input an integer: ")
        user_input = int(user_input_str)
    except ValueError:
        logging.error("Unvalid input, please input an integer.")

    write_spot = memory[i + 1]
    memory[write_spot] = user_input
    return i + 2

def set_output(memory, i, modes):
    output = get_parameter(memory, i + 1, modes[-1])
    return i + 2, output

def jump_if_true(memory, i, modes):
    param1 = get_parameter(memory, i + 1, modes[-1])
    param2 = get_parameter(memory, i + 2, modes[-2])
...
def jump_if_false(memory, i, modes):
    param1 = get_parameter(memory, i + 1, modes[-1])
    param2 = get_parameter(memory, i + 2, modes[-2])
...
def handle_equals(memory, i, modes):
    param1 = get_parameter(memory, i + 1, modes[-1])
    param2 = get_parameter(memory, i + 2, modes[-2])
...
def run_program(program_code):
    ...
    while True:
        opcode, modes = translate_instruction(memory[i])

        if opcode == 99:
            break
            
        elif opcode == 1:
            i = add(memory, i, modes)
            
        elif opcode == 2:
            i = multiply(memory, i, modes)
                
        elif opcode == 3:
            i = user_input(memory, i)
            
        elif opcode == 4:
            i, output = set_output(memory, i, modes)
            output_count += 1
            if output_count > 1:
                logging.error("The input and/or list is unvalid (more then one output was triggerd)")
                return None
                    
        elif opcode == 5:
            i = jump_if_true(memory, i, modes)
                
        elif opcode == 6:
            i = jump_if_false(memory, i, modes)
                
        elif opcode == 7:
            i = less_than(memory, i, modes)
                
        elif opcode == 8:
            i = handle_equals(memory, i, modes)
                
        else:
            logging.error(f"Unknown optcode {opcode} at position {i}")
            return None
...
if __name__ == "__main__":
    ...
    print("output commands: ", run_program(commands), "\n")