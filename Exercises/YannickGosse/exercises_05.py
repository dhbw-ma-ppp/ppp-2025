import reference_int_computer as int_comp
...
def handle_output(value):
    global ball_x, paddle_x, output_buffer, score
    output_buffer.append(value)
    if len(output_buffer) == 3:
        x, y, tile_id = output_buffer
        output_buffer = []
        game_history.append((x, y, tile_id))
        if x == -1 and y == 0:
            score = tile_id
        else:
            if tile_id == 3:
...
def update_frame(i):
    x, y, tile_id = game_history[i]
    
    if x != -1:
        screen_grid[y, x] = tile_id

    img.set_data(screen_grid)
    return img,

script_location = Path(__file__).parent

data_file_path = (script_location / "../../data/breakout_commands.txt").resolve()

with open(data_file_path) as ifile:
    file_content = ifile.readlines()

for values in file_content:
    input_commands.append(int(values.replace("\n", "")))

input_commands[0] = 2
    
Computer = int_comp.IntComputer(get_joystick_input, handle_output)

Computer.run(input_commands)

for x, y, tile_id in game_history:
    if x != -1: 
        if x > max_x: max_x = x
        if y > max_y: max_y = y

screen_grid = np.zeros((max_y + 1, max_x + 1))

fig, ax = plt.subplots()
img = ax.imshow(screen_grid, vmin=0, vmax=4)

anim = animation.FuncAnimation(fig, update_frame, frames=len(game_history), interval=1, blit=True, repeat=False)
