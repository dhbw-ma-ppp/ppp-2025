...
def run_program(commands, input_values): #input immer 5 bei "offizieler Ausführung"
    ...
    def get_value(parameter, mode):
        ...
        elif mode == 1:  # Direktmodus
            return parameter
        else:
            raise ValueError(f"Unbekannter Parameter-Modus {mode} -> Fehler in Programm")

    while True:
        instruction = commands[ip]
        opcode = instruction % 100  # letzte zwei Ziffern
        modes = [(instruction // 100) % 10, # 3. letzte Ziffer 
                 (instruction // 1000) % 10]  #4. letzte Ziffer
        if opcode == 99:
            print("\n99 gefunden!\n")
            break

        if opcode in (1, 2, 7, 8):  # drei Parameter
            p1, p2, p3 = commands[ip+1:ip+4]
            v1 = get_value(p1, modes[0])
            v2 = get_value(p2, modes[1])
            # p3 ist immer Positionsmodus
            if opcode == 1:  # Addition
                commands[p3] = v1 + v2
            elif opcode == 2:  # Multiplikation
                commands[p3] = v1 * v2
            elif opcode == 7:  # Less than
                commands[p3] = 1 if v1 < v2 else 0
            ...
            raise ValueError(f"Unbekannter Opcode {opcode} an Position {ip} -> Fehler in Programm")
